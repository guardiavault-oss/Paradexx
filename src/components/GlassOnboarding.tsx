...
export default function GlassOnboarding({ onComplete, type, onBack }: GlassOnboardingProps) {
  ...
  const [usedGoogleAuth, setUsedGoogleAuth] = useState(false); // Track if user used Google Auth
  ...
  const handleNext = () => {
    // Easy flow with Google Auth: 1 → 2 → 3 → 6 → 7 → 8 (skip password & verify)
    // Easy flow without Google (guest): 1 → 2 → 3 → 4 → 6 → 7 → 8 (need password, skip verify)
    // Advanced flow: 1 → 2 → 3 → 4 → 5 → 6 → 7 → 8

    let nextStep = step + 1;

    if (data.walletSetupType === "easy") {
      if (step === 3) {
        // After Google Auth step
        if (usedGoogleAuth) {
          nextStep = 6; // Skip password AND verify - go to biometrics
        } else {
          nextStep = 4; // Need password step for guests
        }
      } else if (step === 4) {
        // After password step (for guests), skip verify email
        nextStep = 6; // Go to biometrics
      }
    }

    // Generate seed phrase when moving to step 7 with easy setup
    if (nextStep === 7 && data.walletSetupType === "easy" && seedPhrase.length === 0) {
  ...
  const handleBack = () => {
    if (step === 1 && onBack) {
      ...
      // For seeded (easy) route: handle back navigation
      if (data.walletSetupType === "easy") {
        if (step === 6) {
          // Going back from biometrics
          if (usedGoogleAuth) {
            prevStep = 3; // Jump back to Google Auth
          } else {
            prevStep = 4; // Jump back to password (for guests)
          }
        } else if (step === 4) {
          prevStep = 3; // Back to Google Auth step
        }
      }
      ...
      try {
        await fetch(`${API_URL}/api/guardians/invite`, {
          ...
          body: JSON.stringify({
            guardianName,
            guardianEmail,
            ownerName: data.name || 'A Paradex user',
          }),
        });
        console.log(`Guardian invitation sent to ${guardianEmail}`);
      } catch (error) {
        console.error('Failed to send guardian invitation:', error);
        // Still proceed - email is just a notification
      }
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 3:
        // For seeded (easy): skip email, will use Google Auth
        // For seedless (advanced): need email
        if (data.walletSetupType === "easy") {
          return true; // Will use Google Auth
        }
        return data.email.includes("@");
      case 4:
        // For seeded: skip password (use Google Auth)
        // For seedless: need password
        if (data.walletSetupType === "easy") {
          return true;
        }
        return data.password.length >= 8;
      case 5:
        // Skip verification for seeded route
        if (data.walletSetupType === "easy") {
          return true;
        }
      ...
      case 7:
        if (data.walletSetupType === "easy") {
          ...
          // Seedless requires minimum 3 guardians
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 3:
        // For seeded (easy) route: Google Auth OR email signup
        // For seedless (advanced) route: Email only
        if (data.walletSetupType === "easy") {
          return (
            <div className="space-y-6 max-w-md mx-auto">
              <div className="text-center mb-8">
                ...
                <p style={{ color: "rgba(255, 255, 255, 0.7)" }}>
                  Sign up with Google or enter your email
                </p>
              </div>
              ...
              {data.email.includes("@") && (
                <button
                  onClick={() => {
                    // Mark that user is using email/password
                    setUsedGoogleAuth(false);
                    handleNext();
                  }}
          ...
          return (
            <div className="space-y-8 max-w-2xl mx-auto">
              <div className="text-center mb-8">
                ...
                <p style={{ color: "rgba(255, 255, 255, 0.7)" }}>
                  Add 3-5 trusted contacts who can help you recover your wallet
                </p>
              </div>
