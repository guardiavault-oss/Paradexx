          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/50">Pre-execution risk analysis</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:divide-x divide-white/10">
          ...
          <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0x..."
                  className="w-full bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-white/30 outline-none focus:border-white/30 transition-colors"
                />
              </div>
            </div>
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="0.0"
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/30 outline-none focus:border-white/30 transition-colors"
                  />
                  <button
                    onClick={handleMaxAmount}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-black uppercase tracking-wider hover:text-white transition-colors"
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white hover:border-white/20 transition-colors min-w-[120px]"
                  >
                  ...
                  <AnimatePresence>
                    {showTokenDropdown && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute right-0 top-full mt-2 w-48 bg-black/95 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden z-50 shadow-xl"
                      >
                        {allTokens.map((token) => (
                          <button
                            key={token.address}
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col items-center gap-1">
                        <Icon
                          className={`w-4 h-4 ${isSelected ? config.color : 'text-white/40'}`}
                        />
                        <span
                          className={`text-xs font-medium ${isSelected ? 'text-white' : 'text-white/50'}`}
                        >
                          {config.label}
                        </span>
                        <span className="text-[10px] text-white/40">{config.time}</span>
                      </div>
                    </motion.button>
                  );
                })}
            ...
            <motion.button
              ...
              className="w-full py-3.5 rounded-xl font-black uppercase tracking-wider text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-white"
              style={{
            ...
            <AnimatePresence mode="wait">
              {!simulationResult && !isSimulating ? (
                ...
                >
                  ...
                  <h3 className="text-sm font-bold text-white/50 mb-2 uppercase tracking-wider">
                    No Simulation Yet
                  </h3>
                ...
                >
                  ...
                  <div className="mt-6 space-y-2 text-center">
                    <p className="text-sm text-white font-bold uppercase tracking-wider">Simulating Transaction</p>
                    <p className="text-xs text-white/50">Analyzing risks and state changes...</p>
                  </div>
                </motion.div>
              ) : simulationResult ? (
                <motion.div
                ...
                >
                  ...
                  <div className="p-4 rounded-xl bg-black/40 border border-white/10">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-xs text-white/40">{simulationResult.gasPriceGwei} Gwei</span>
                    </div>
                      ...
                      <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          className={`h-full rounded-full ${
                            simulationResult.riskAssessment.score <= 30
                              ? 'bg-green-500'
                        ...
                        {simulationResult.balanceChanges.map((change, idx) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-white font-bold">{change.symbol}</span>
                            </div>
                  ...
                  <motion.button
                    ...
                    className="w-full py-3.5 rounded-xl font-black uppercase tracking-wider text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-green-600 hover:bg-green-500 text-white"
                  >
