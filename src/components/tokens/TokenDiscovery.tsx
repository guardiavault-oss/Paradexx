...
interface TokenDiscoveryProps {
  onTrade: (token: DisplayToken) => void;
  onAnalyze: (token: DisplayToken) => void;
  type?: 'degen' | 'regen';
}
  ...
  const fetchTrendingTokens = useCallback(async () => {
    try {
      ...
      if (response.success && response.data && response.data.length > 0) {
        const displayTokens: DisplayToken[] = response.data.map((token: {
          symbol?: string;
          name?: string;
          logo?: string;
          image?: string;
          ...
          priceChange24h: token.change24h || token.priceChange24h || 0,
          marketCapRank: token.marketCapRank || token.rank || 0,
          chain: token.chain || "ETH",
          tokenId: token.id || token.address || token.symbol,
        }));
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unable to load trending tokens. Please try again.';
