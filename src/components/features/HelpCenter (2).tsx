          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {HELP_CATEGORIES.map((category) => (
              <motion.button
                ...
                className="p-4 bg-white/5 backdrop-blur-xl rounded-xl border border-white/10 hover:border-white/20 transition-all text-left"
              >
                <div
        ...
        {article.content.split("\n").map((paragraph, i) => {
          if (
            ...
            return (
              <h3
                key={i}
                className="text-lg font-black text-white uppercase tracking-tight mt-6 mb-3"
              >
                {paragraph.replace(/\*\*/g, "")}
              </h3>
            );
          ...
          if (paragraph.startsWith("💡")) {
            return (
              ...
              >
                <p className="text-white/90">{paragraph}</p>
              </div>
            );
          }
          if (paragraph.trim()) {
            return (
              <p key={i} className="text-white/70 my-3">
                {paragraph}
              </p>
            );
          }
...
function ArticleCard({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className="w-full p-4 bg-white/5 backdrop-blur-xl rounded-xl border border-white/10 hover:border-white/20 transition-all text-left"
    >
