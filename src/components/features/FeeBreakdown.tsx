...
export function FeeBreakdown({
  ...
  return (
    <div
      className={`bg-white/5 backdrop-blur-xl rounded-2xl border border-[var(--border-neutral)]/10 overflow-hidden ${className}`}
    >
      {/* Primary Display - "You Receive" */}
      <div className="p-6 border-b border-[var(--border-neutral)]/10">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-[var(--text-primary)]/50 uppercase tracking-wider font-bold">
            You receive
          </span>
          {type !== "wrap" && (
        ...
        <div className="flex items-baseline gap-3">
          ...
          <span className="text-xl text-[var(--text-primary)]/50 uppercase tracking-tight">
            {outputToken}
          </span>
        </div>
        <span className="text-sm text-[var(--text-primary)]/40 mt-1 block">
          ≈ ${outputValueUSD}
        </span>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <span className="text-sm text-[var(--text-primary)]/50 uppercase tracking-wider font-bold block">
              Total fees
            </span>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <ChevronDown className="w-5 h-5 text-[var(--text-primary)]/40" />
          </motion.div>
        </div>
      </motion.button>
        ...
        {isExpanded && (
          ...
          >
            <div className="px-6 pb-6 space-y-4">
              ...
              {/* Divider */}
              <div className="border-t border-[var(--border-neutral)]/10 pt-4">
...
function FeeRow({
  ...
  const getIcon = () => {
    switch (item.type) {
      ...
      default:
        return <Info className="w-5 h-5 text-[var(--text-primary)]/50" />;
    }
  };
  ...
  return (
    <div className="flex items-center justify-between relative group">
      <div className="flex items-center gap-3">
        ...
        <div>
          <span className="text-sm text-[var(--text-primary)] font-bold block">
            {item.label}
          </span>
          <span className="text-xs text-[var(--text-primary)]/40 uppercase tracking-wider">
            {item.amount}
          </span>
        </div>
        {item.tooltip && (
          <div className="relative">
            <motion.button
              ...
              className="p-1"
            >
              <Info className="w-4 h-4 text-[var(--text-primary)]/30" />
            </motion.button>
            ...
            <AnimatePresence>
              {showTooltip && (
                <motion.div
                  ...
                  className="absolute left-0 top-8 z-10 p-3 bg-[var(--bg-base)]/95 backdrop-blur-xl border border-[var(--border-neutral)]/10 rounded-xl text-xs text-[var(--text-primary)]/80 max-w-[250px] shadow-2xl"
                >
                  {item.tooltip}
                </motion.div>
              )}
    ...
    <div className="flex items-center gap-3 p-4 bg-white/5 rounded-xl">
      ...
      <div className="flex gap-2">
        {speeds.map((s) => (
          <span
            key={s.id}
            className={`px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all ${s.active
                ? "text-[var(--text-primary)]"
                : "bg-white/5 text-[var(--text-primary)]/40"
              }`}
            style={
              s.active
                ? {
      ...
      <span className="text-xs text-[var(--text-primary)]/50 ml-auto uppercase tracking-wider font-bold">
        {time}
      </span>
    </div>
  );
...
export function InlineFeeDisplay({
  ...
  return (
    <div className="flex items-center gap-2 text-xs text-[var(--text-primary)]/50">
      ...
      <span className="text-[var(--text-primary)]/40">(~${feeUSD})</span>
    </div>
  );
}
