...
export function NotificationCenter({ type, onClose }: NotificationCenterProps) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className="bg-black/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden"
    >
      {/* Header */}
      <div className="p-4 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bell className="w-5 h-5" style={{ color: accentColor }} />
          <h3 className="text-base font-black uppercase tracking-tight text-white">Notifications</h3>
        </div>
        <button
          onClick={onClose}
      ...
      {mockNotifications.length > 0 && (
        <div className="p-3 border-t border-white/10">
          <button
            className="w-full text-center text-xs font-bold uppercase tracking-wider hover:text-white transition-colors"
