...
const PageTransition: React.FC<{
  ...
  useEffect(() => {
    // Only play transition if triggerKey > 0 (skips login, only for tribe switching)
    if (!isActive || triggerKey === 0) {
...
export default function App() {
  const [showWalletEntry, setShowWalletEntry] = useState(true); // Start with wallet entry
  ...
  const handleLogin = (email: string, password: string) => {
    ...
    // Set user data and go straight to dashboard
  ...
  const handleTribeSwitch = () => {
    // Switch between degen and regen
    const newTribe = selectedSide === "degen" ? "regen" : "degen";
    setSelectedSide(newTribe);
    
    // Update assessment results with swapped percentages
    if (assessmentResults) {
      setAssessmentResults({
        ...assessmentResults,
        tribe: newTribe,
        degenPercent: assessmentResults.regenPercent,
        regenPercent: assessmentResults.degenPercent,
      });
    }
  };
  ...
  const handleOnboardingComplete = async (data: any, seedPhrase?: string[]) => {
    ...
    // After wallet creation, show the split screen to choose path
    setShowOnboarding(false);
    // Split screen will show when selectedSide is null
  };
              ...
              {showTerms ? (
                ...
                <Suspense fallback={<div className="w-full h-screen bg-black" />}>
                  <DashboardNew
                    ...
                    onTribeSwitch={handleTribeSwitch}
                  />
                </Suspense>
              ) : !selectedSide ? (
                <SplitScreenView
...
// Import router at the end to avoid circular dependencies
const AppRouterModule = await import("./AppRouter");
