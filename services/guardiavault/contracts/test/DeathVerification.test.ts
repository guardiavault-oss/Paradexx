...
describe("Death Verification Oracle Integration", function () {
  ...
  describe("Oracle Death Verification", function () {
    it("Should allow oracle to verify death", async function () {
      ...
      await expect(
        guardiaVault.connect(oracle).verifyDeath(owner.address)
      )
        .to.emit(guardiaVault, "DeathVerified")
        .withArgs(
          vaultId,
          owner.address,
          oracle.address,
          (timestamp: any) => timestamp > 0,
          (readyAt: any) => readyAt > 0
        );

      // Check vault status is DeathVerified
      const status = await guardiaVault.getVaultStatus(vaultId);
      expect(status).to.equal(VaultStatus.DeathVerified);
    });
    ...
    it("Should transition from DeathVerified to ReadyForClaim after delay", async function () {
      ...
      // Check vault status is ReadyForClaim
      const status = await guardiaVault.getVaultStatus(vaultId);
      expect(status).to.equal(VaultStatus.ReadyForClaim);
    });
    ...
    it("Should allow beneficiary to claim when ReadyForClaim", async function () {
      ...
      // Beneficiary claims
      await expect(
        guardiaVault.connect(beneficiary).claim(vaultId)
      )
        .to.emit(guardiaVault, "BeneficiaryClaimed")
        .withArgs(vaultId, beneficiary.address, (timestamp: any) => timestamp > 0, metadataHash);
    });
    ...
    it("Should reject non-oracle from verifying death", async function () {
      ...
      await expect(
        guardiaVault.connect(guardian1).verifyDeath(owner.address)
      ).to.be.revertedWithCustomError(guardiaVault, "OnlyOracle");
    });
    ...
    it("Should prevent double verification", async function () {
      ...
      await expect(
        guardiaVault.connect(oracle).verifyDeath(owner.address)
      ).to.be.revertedWithCustomError(guardiaVault, "DeathAlreadyVerified");
    });
    ...
    it("Should emit DeathVerified event with correct parameters", async function () {
      ...
      expect(parsedDeathEvent?.args[3]).to.be.gt(0); // timestamp
      expect(parsedDeathEvent?.args[4]).to.be.gt(0); // readyForClaimAt
    });
  });
  ...
  describe("Integration: Complete Flow", function () {
    it("Should complete full flow: DeathVerified → ReadyForClaim → Claimed", async function () {
      ...
      expect(status).to.equal(VaultStatus.Active);
      ...
      // Status should be ReadyForClaim
      status = await guardiaVault.getVaultStatus(vaultId);
      expect(status).to.equal(VaultStatus.ReadyForClaim);
      ...
      status = await guardiaVault.getVaultStatus(vaultId);
      expect(status).to.equal(VaultStatus.Claimed);
