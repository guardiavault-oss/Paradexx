...
import { splitSecret, combineShares } from '../server/services/shamir.js';
...
async function testRecovery(scheme: string, fragments: string[], vaultId: string | null = null): Promise<TestResult> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`   Error: ${(data as any).message || JSON.stringify(data)}`);
...
async function runTests() {
  ...
  // Test 4: Legacy 3-of-5 with exactly 3 fragments (simulated as 5)
  console.log('\n' + '='.repeat(60));
  console.log('TEST 4: Legacy 3-of-5 scheme with 3 fragments (5 provided for detection)');
  console.log('='.repeat(60));
  try {
    const result35 = splitSecret(TEST_SECRET, 3, 5);
    const fragments35 = result35.shares.slice(0, 3);
    // Add 2 more to trigger 3-of-5 detection
